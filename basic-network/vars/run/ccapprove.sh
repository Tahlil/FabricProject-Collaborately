...
peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("collaborately-contract_1.0:"))' > ccstatus.json

PKID=$(jq '.package_id' ccstatus.json | xargs)
REF=$(jq '.references.globalchannel' ccstatus.json)

SID=$(peer lifecycle chaincode querycommitted -C globalchannel -O json \
  | jq -r '.chaincode_definitions|.[]|select(.name=="collaborately-contract")|.sequence' || true)
if [[ -z $SID ]]; then
  SEQUENCE=1
elif [[ -z $REF ]]; then
  SEQUENCE=$SID
...
# approved=$(peer lifecycle chaincode checkcommitreadiness --channelID globalchannel \
#   --name collaborately-contract --version 1.0 --init-required --sequence $SEQUENCE --tls \
#   --cafile $ORDERER_TLS_CA --output json | jq -r '.approvals.org0-example-com')

# if [[ "$approved" == "false" ]]; then
  peer lifecycle chaincode approveformyorg --channelID globalchannel --name collaborately-contract \
